...
class DubFragment : BaseFragment(), IAlarmTabCall {
    //初始化打开首页
    ...
    override fun itemTabSelected(page: Int, isRequest: Boolean, tabType: Int) {
        ...
        if (!isRequest || isCurrent) {
            ...
            playBean = null
            lastId = ""
            tagId = if (tabType == 0) "" else tabType.toString()
            request(0)
        }
        //请求数据
    }
    ...
    override fun request(flag: Int) {
        OkClientHelper.get(activity, "dubbings?lastId=$lastId&tagId=$tagId", WordingData::class.java, object : OkResponse {
            override fun success(result: Any?) {
