...
class UserMovieFragment : BaseFragment() {
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<BaseSearchBean>(activity, R.layout.item_user_movies, mData) {
            override fun convert(helper: BaseAdapterHelper?, item: BaseSearchBean?) {
                Glide.with(this@UserMovieFragment)
                ...
                helper.getTextView(R.id.tv_UserComment).text = if (loginBean.user_id != userId) resources.getString(R.string.string_movies_other_comment) else resources.getString(R.string.string_movies_self_comment)
    ...
    override fun request(flag: Int) {
        OkClientHelper.get(activity, "users/$userId/resources/1?lastId=$lastId${if (TextUtils.isEmpty(score)) "" else "&score=$score"}", UserMoviesData::class.java, object : OkResponse {
            override fun success(result: Any?) {
                ...
                allcount.text = "共${result.data.total}部"
