...
class SearchMovieActivity : BaseAct() {
    lateinit var adapter: QuickAdapter<String>
        ...
        etContent.setOnEditorActionListener { v, actionId, event ->
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                ...
                searchTag?.let {
                    ...
                    startActivity(Intent(this@SearchMovieActivity, SearchMovieResultActivity::class.java).putExtra("tag", searchTag))
                }
            }
            false
        }
        adapter.setOnItemClickListener { view, position ->
            startActivity(Intent(this@SearchMovieActivity, SearchMovieResultActivity::class.java).putExtra("tag", mData[position]))
        }
    }
    ...
    private fun getHistory(limit: Int) {
        mData.clear()
        val localTag = SPUtils.getString(this, IConstant.MOVIE, "")
    ...
    private fun updateHistory(tag: String, isMove: Boolean) {
        var localTag = SPUtils.getString(this, IConstant.MOVIE, "")
        if (!isMove) {
            val split = localTag.split("__".toRegex())
            for (str in split) {
                if (tag == str)
        ...
        SPUtils.setString(this, IConstant.MOVIE, localTag)
        getHistory(3)
    }
