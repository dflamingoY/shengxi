...
import kotlinx.android.synthetic.main.activity_similar_white.*
...
class SimilarCommonMoviesActivity : BaseAct(), ItemOperator {
    ...
    private var similarUid: String? = null
    private lateinit var audioPlayer: AudioPlayer
    private var playBean: BaseBean? = null
    private var lastId: String = ""
    private var resourceType = 1//1电影 2 书籍 3 音乐
    ...
    override fun initView() {
        ...
        headView = LayoutInflater.from(this).inflate(R.layout.head_user_movies, recyclerReview, false) as TextView
    }

    @SuppressLint("SetTextI18n")
    override fun initData() {
        resourceType = intent.getIntExtra("resourceType", 1)
        tv_Title.text = if (resourceType == 1) resources.getString(R.string.string_similar_movies_title) else if (resourceType == 2) resources.getString(R.string.string_book_similar_match_1) else resources.getString(R.string.string_music_similar_match_1)
        setItemOperator(this)
        similarUid = intent.getStringExtra("uid")
        val nickName = intent.getStringExtra("nickName")
        headView.text = String.format(when (resourceType) {
            1 -> resources.getString(R.string.string_movies_similar_match)
            2 -> resources.getString(R.string.string_book_similar_match)
            else -> resources.getString(R.string.string_music_similar_match)
        }, nickName)
        ...
        adapter = object : QuickAdapter<BaseBean>(this, R.layout.item_voice, mData, headView) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: BaseBean?) {
                ...
                helper.getTextView(R.id.tvTime)?.text = TimeUtils.getInstance().paserFriends(this@SimilarCommonMoviesActivity, item.created_at)
                ...
                if (!TextUtils.isEmpty(item.is_shared)) {
                    ...
                    params.width = AppTools.dp2px(this@SimilarCommonMoviesActivity, 27)
                    params.height = AppTools.dp2px(this@SimilarCommonMoviesActivity, 27)
                ...
                helper.getView(R.id.lineaer_Recommend)?.setOnClickListener {
                    if (!TextUtils.isEmpty(item.is_shared)) {
                        startActivity(Intent(this@SimilarCommonMoviesActivity, DynamicDetailsActivity::class.java)
                        ...
                        if (item.dialog_num == 0) {
                            ...
                            startActivity(Intent(this@SimilarCommonMoviesActivity, TalkListActivity::class.java)
                ...
                helper.getView(R.id.relativeShare)?.setOnClickListener {
                    if (!TextUtils.isEmpty(item.is_shared))
                        DialogMore(this@SimilarCommonMoviesActivity).setPrivacyStatus(item.is_private).setOnClickListener(View.OnClickListener {
                            when (it.id) {
                                R.id.tv_ShareWechat -> {
                                    DialogShare(this@SimilarCommonMoviesActivity, true).setOnClickListener(View.OnClickListener { share ->
                                        when (share.id) {
                                            R.id.linearWechat -> {
                                                ShareUtils.share(this@SimilarCommonMoviesActivity, Wechat.NAME, item.voice_url, item.share_url, "", item.user)
                                            }
                                            R.id.linearMoment -> {
                                                ShareUtils.share(this@SimilarCommonMoviesActivity, WechatMoments.NAME, item.voice_url, item.share_url, "", item.user)
                                            }
                                        }
                                    }).show()
                                }
                                R.id.tv_ShareWeibo -> {
                                    ShareUtils.share(this@SimilarCommonMoviesActivity, SinaWeibo.NAME, item.voice_url, item.share_url, "", item.user)
                                }
                                R.id.tv_ShareQQ -> {
                                    DialogShare(this@SimilarCommonMoviesActivity, false).setOnClickListener(View.OnClickListener { share ->
                                        when (share.id) {
                                            R.id.linearQQ -> {
                                                ShareUtils.share(this@SimilarCommonMoviesActivity, QQ.NAME, item.voice_url, item.share_url, "", item.user)
                                            }
                                            R.id.linearQzone -> {
                                                ShareUtils.share(this@SimilarCommonMoviesActivity, QZone.NAME, item.voice_url, item.share_url, "", item.user)
                                            }
                                        }
                                    }).show()
                                }
                                R.id.tv_Delete -> {
                                    DialogDeleteConment(this@SimilarCommonMoviesActivity).setHintText(resources.getString(R.string.string_delete_voice)).setOnClickListener(View.OnClickListener {
                                ...
                                R.id.tv_Self -> {
                                    if (item.is_private == 1) {
                                        ...
                                        DialogPrivacy(this@SimilarCommonMoviesActivity).setOnClickListener(View.OnClickListener {
                                            transLayout.showProgress()
                                            setVoicePrivacy(item)
                                        }).show()
                                    }
                                ...
                                R.id.tv_add_album -> {
                                    startActivity(Intent(this@SimilarCommonMoviesActivity, DialogAddAlbumActivity::class.java).putExtra("voiceId", item.voice_id))
                                }
                            }
                        }).show()
                    else {
                        DialogReport(this@SimilarCommonMoviesActivity).setResource(item.resource_type, item.subscription_id != 0).setOnClickListener(View.OnClickListener { report ->
                            when (report.id) {
                                R.id.tv_admin_setPrivacy -> {
                                    dialogPwd = DialogCommitPwd(this@SimilarCommonMoviesActivity).setOperator("isHidden", "1").setOnResultListener(DialogAdminPwdListener { key, value, pwd ->
                                        adminSetPrivacy(FormBody.Builder().add(key, value).add("confirmPasswd", pwd).build(), item)
                                    })
                                    dialogPwd?.show()
                                }
                                R.id.tv_Report ->
                                    DialogReportContent(this@SimilarCommonMoviesActivity).setOnResultListener(OnReportItemListener {
                                        transLayout.showProgress()
                                        report(item.voice_id, it, item.user.id)
                                    }).show()
                                R.id.tv_report_normal -> {
                                    DialogNormalReport(this@SimilarCommonMoviesActivity).setOnClickListener(View.OnClickListener { report ->
                                        when (report.id) {
                                            R.id.tv_Attach -> {
                                ...
                                R.id.tv_Follow -> {
                                    if (item.subscription_id == 0) {
                                        ...
                                        DialogGraffiti(this@SimilarCommonMoviesActivity).setTitle(String.format(resources.getString(R.string.string_follow_explor1), when (item.resource_type) {
...
//                            transLayout.showProgress()
                ...
                helper.getImageView(R.id.roundImg)?.setOnClickListener { UserDetailsActivity.start(this@SimilarCommonMoviesActivity, item.user.avatar_url, item.user.id, helper.getImageView(R.id.roundImg)) }
            ...
            private fun download(helper: BaseAdapterHelper, item: BaseBean) {
                try {
                    ...
                    if (file.exists()) {
                        ...
                        audioPlayer.start(if (SPUtils.getBoolean(this@SimilarCommonMoviesActivity, IConstant.ISEARPIECE, false)) AudioManager.STREAM_VOICE_CALL else AudioManager.STREAM_MUSIC)
                    } else {
                        OkClientHelper.downFile(this@SimilarCommonMoviesActivity, item.voice_url, { o ->
                            try {
                                if (null == o) {
                                    showToast(resources.getString(R.string.string_error_file))
                                    return@downFile
                    ...
                    cache.loop { baseAda ->
                        ...
                        if (TextUtils.isEmpty(b.is_shared)) {
                            helper.getView(R.id.iv_Thumb).let {
                                ...
                                SmallBang.attach2Window(this@SimilarCommonMoviesActivity).bang(it, 60f, null)
                            }
                        }
                    }
                }
    ...
    override fun request(flag: Int) {
        OkClientHelper.get(this, "hobby/common/$resourceType/users/$similarUid?lastId=$lastId", VoiceData::class.java, object : OkResponse {
