...
import org.xiaoxingqi.shengxi.modules.listen.alarm.downFile
...
class MoodSongsFragment : BaseFragment() {
    ...
    override fun initView(view: View?) {
        view!!.relativePlayMenu.visibility = View.VISIBLE
        ObjectAnimator.ofFloat(view.relativePlayMenu, "translationY", AppTools.dp2px(context, 56).toFloat()).setDuration(0).start()
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<BaseSearchBean>(activity, R.layout.item_user_music, mData) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: BaseSearchBean?) {
                ...
                Glide.with(this@MoodSongsFragment)
                ...
                helper.getTextView(R.id.tv_MovieName).text = item.song_name
                ...
                helper.getView(R.id.ivPlay).setOnClickListener {
                    ...
                    mView!!.relativePlayMenu.anim(320L)
                }
        ...
        arguments?.let {
            ...
            if (loginBean.user_id != userId) {
                mView!!.transLayout.tvDesc.text = if (it.getInt("relation") != 2 && permission == 2) {
                    ...
                    resources.getString(R.string.string_resource_empty_11)
                } else {
                    request(0)
                    "ta${resources.getString(R.string.string_music_3)}"
                }
            } else {
                isSelf = true
                request(0)
    ...
    override fun request(flag: Int) {
        OkClientHelper.get(activity, "users/$userId/resources/3?lastId=$lastId", UserMoviesData::class.java, object : OkResponse {
            @SuppressLint("SetTextI18n")
