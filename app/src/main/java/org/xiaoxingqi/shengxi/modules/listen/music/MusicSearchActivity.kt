...
class MusicSearchActivity : BaseAct() {
    ...
    override fun initView() {
        ...
        etContent.hint = resources.getString(R.string.string_search_music_or_singer)
    }
        ...
        etContent.setOnEditorActionListener { _, actionId, _ ->
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                ...
                searchTag?.let {
                    ...
                    startActivity(Intent(this@MusicSearchActivity, MusicSearchResultActivity::class.java).putExtra("tag", searchTag))
                }
            }
            false
        }
        adapter.setOnItemClickListener { view, position ->
            startActivity(Intent(this@MusicSearchActivity, MusicSearchResultActivity::class.java).putExtra("tag", mData[position]))
        }
    }
    ...
    private fun getHistory(limit: Int) {
        mData.clear()
        val localTag = SPUtils.getString(this, IConstant.MUSIC, "")
    ...
    private fun updateHistory(tag: String, isMove: Boolean) {
        var localTag = SPUtils.getString(this, IConstant.MUSIC, "")
        if (!isMove) {
            val split = localTag.split("__".toRegex())
            for (str in split) {
                if (tag == str)
        ...
        SPUtils.setString(this, IConstant.MUSIC, localTag)
        getHistory(3)
    }
}