 ...
 * 用户首3条私聊
 */
class UserChatFragment : BaseFragment(), ITabClickCall {
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<TalkListData.TalkListBean>(activity, R.layout.item_admin_voice_list, mData) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: TalkListData.TalkListBean?) {
                ...
                helper.getTextView(R.id.tv_Sub).text = "查看对象"
...
//                helper.getView(R.id.linear_content).visibility = if (TextUtils.isEmpty(item.resource_content)) View.GONE else View.VISIBLE
                ...
                if (item.resource_type == "1") {//语音
                    ...
                    helper.getView(R.id.relative_img).visibility = View.VISIBLE
                    helper.getView(R.id.linear_content).visibility = View.GONE
                    progress.visibility = View.GONE
                    Glide.with(this@UserChatFragment)
                            .applyDefaultRequestOptions(RequestOptions().signature(ObjectKey(item.resource_url)))
                            .asBitmap()
                            .load(item.resource_url)
                            .into(helper.getImageView(R.id.iv_chat_content))
                    helper.getImageView(R.id.iv_chat_content).setOnClickListener {
    ...
    private fun setReadTag(bean: TalkListData.TalkListBean, position: Int) {
        ...
        OkClientHelper.post(activity, "admin/${loginBean.user_id}/point", FormBody.Builder().add("pointKey", "chatPriPoint")
    ...
    override fun request(flag: Int) {
        when (flag) {
            0 -> {
                val loginBean = PreferenceTools.getObj(activity, IConstant.LOCALTOKEN, LoginData.LoginBean::class.java)
                OkClientHelper.get(activity, "admin/${loginBean.user_id}/point/chatPriPoint", TestEnjoyCountData::class.java, object : OkResponse {
                    override fun success(result: Any?) {
            ...
            1 -> {
                OkClientHelper.get(activity, "admin/dialog?token=${SPUtils.getString(activity, IConstant.ADMINTOKEN, "")}&chatType=2&lastId=$lastId", TalkListData::class.java, object : OkResponse {
                    override fun success(result: Any?) {
