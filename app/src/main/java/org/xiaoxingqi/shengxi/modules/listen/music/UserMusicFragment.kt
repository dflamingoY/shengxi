...
class UserMusicFragment : BaseFragment() {
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<BaseSearchBean>(activity, R.layout.item_user_music, mData) {
            override fun convert(helper: BaseAdapterHelper?, item: BaseSearchBean?) {
                Glide.with(this@UserMusicFragment)
                ...
                helper.getTextView(R.id.tv_UserComment).text = if (loginBean.user_id != userId) "Ta的版本" else "我的版本"
    ...
    override fun request(flag: Int) {
        OkClientHelper.get(activity, "users/$userId/resources/3?lastId=$lastId${if (TextUtils.isEmpty(score)) "" else "&score=$score"}", UserMoviesData::class.java, object : OkResponse {
            override fun success(result: Any?) {
                ...
                allcount.text = "共${result.data.total}首"
