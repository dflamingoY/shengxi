...
class MoodBooksFragment : BaseFragment() {
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<BaseSearchBean>(activity, R.layout.item_user_movies, mData) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: BaseSearchBean?) {
                Glide.with(this@MoodBooksFragment)
        ...
        arguments?.let {
            ...
            mView!!.transLayout.tvDesc.text = if (loginBean.user_id != userId) {
                if (it.getInt("relation") != 2 && permission == 2) {
                    mView!!.transLayout.showEmpty()
                    mView!!.swipeRefresh.isEnabled = false
                    resources.getString(R.string.string_resource_empty_9)
                } else {
                    request(0)
                    "ta${resources.getString(R.string.string_resource_empty_13)}"
                }
            } else {
                request(0)
                isSelf = true
                resources.getString(R.string.string_resource_empty_13)
            }
        }
    }
    ...
    */
    ...
    override fun request(flag: Int) {
        OkClientHelper.get(activity, "users/$userId/resources/2?lastId=$lastId", UserMoviesData::class.java, object : OkResponse {
