...
import kotlinx.android.synthetic.main.layout_book_head_details.view.*
...
class OneBookDetailsActivity : BaseAct(), ItemOperator {
    ...
    private var bookData: BaseSearchBean? = null
    ...
    override fun initView() {
        ...
        headView = LayoutInflater.from(this).inflate(R.layout.layout_book_head_details, recyclerView, false)
        tv_Title.text = resources.getString(R.string.string_one_book_detais_title)
        try {
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<BaseBean>(this, R.layout.item_voice, mData, headView) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: BaseBean?) {
                item!!.resourceContent(helper!!, this@OneBookDetailsActivity, glideUtil, 0)
                ...
                item.echoClick(this@OneBookDetailsActivity, helper) {
                ...
                helper.getView(R.id.tv_push).setOnClickListener {
                    if (item.subscription_id == 0) {
                        ...
                        DialogGraffiti(this@OneBookDetailsActivity).setTitle(String.format(resources.getString(R.string.string_follow_explor1), resources.getString(R.string.string_follow_book)), resources.getString(R.string.string_confirm)).setOnClickListener(View.OnClickListener {
                ...
                helper.getView(R.id.relativeShare).setOnClickListener {
                    if (!TextUtils.isEmpty(item.is_shared))
                        shareDialog(item) { id ->
                            when (id) {
                                R.id.tv_Delete -> {
                                    DialogDeleteConment(this@OneBookDetailsActivity).setHintText(resources.getString(R.string.string_delete_voice)).setOnClickListener(View.OnClickListener {
                                ...
                                R.id.tv_Self -> {
                                    if (item.is_private == 1) {
                                        ...
                                        DialogPrivacy(this@OneBookDetailsActivity).setOnClickListener(View.OnClickListener {
                                            transLayout.showProgress()
                                            setVoicePrivacy(item)
                                        }).show()
                                    }
                        ...
                        DialogReport(this@OneBookDetailsActivity).setResource(item.resource_type, item.subscription_id != 0).setOnClickListener(View.OnClickListener { report ->
                            when (report.id) {
                                R.id.tv_admin_setPrivacy -> {
                                    dialogPwd = DialogCommitPwd(this@OneBookDetailsActivity).setOperator("isHidden", "1").setOnResultListener(DialogAdminPwdListener { key, value, pwd ->
                                        adminSetPrivacy(FormBody.Builder().add(key, value).add("confirmPasswd", pwd).build(), item)
                                    })
                                    dialogPwd?.show()
                                }
                                R.id.tv_Report ->
                                    DialogReportContent(this@OneBookDetailsActivity).setOnResultListener(OnReportItemListener {
                                        transLayout.showProgress()
...
//                            transLayout.showProgress()
                ...
                helper.getImageView(R.id.roundImg)?.setOnClickListener { UserDetailsActivity.start(this@OneBookDetailsActivity, item.user.avatar_url, item.user.id, helper.getImageView(R.id.roundImg)) }
                        ...
                        if (TextUtils.isEmpty(b.is_shared)) {
                            helper.getView(R.id.iv_Thumb).let {
                                ...
                                if (recyclerView.isOffsetScreen(mData.indexOf(bean) + 1)) {
                                    SmallBang.attach2Window(this@OneBookDetailsActivity).bang(it, 60f, null)
                                }
                            }
                        }
                    }
    ...
    private fun down(item: BaseBean, isScroll: Boolean = true) {
        if (isScroll && recyclerView.scrollState == RecyclerView.SCROLL_STATE_IDLE) {
            recyclerView.scroll(this@OneBookDetailsActivity, mData.indexOf(item) + 1)
        }
        ...
        try {
            ...
            if (file.exists()) {
                ...
                audioPlayer.start(if (SPUtils.getBoolean(this@OneBookDetailsActivity, IConstant.ISEARPIECE, false)) AudioManager.STREAM_VOICE_CALL else AudioManager.STREAM_MUSIC)
            } else {
                OkClientHelper.downFile(this@OneBookDetailsActivity, item.voice_url, { o ->
                    try {
                        if (null == o) {
                            showToast(resources.getString(R.string.string_error_file))
                            return@downFile
...
//                    showToast(VolleyErrorHelper.getMessage(it))
    ...
    private fun nextAudio() {
        playBean?.let {
            if (SPUtils.getBoolean(this@OneBookDetailsActivity, IConstant.PLAY_MENU_AUTO, false)) {
    ...
    override fun initEvent() {
        ...
        customPlayMenu.setOnCircleMenuListener(object : OnCircleMenuOperatorListener {
            ...
            override fun top() {
                if (!audioPlayer.isPlaying) {
                    ...
                    playBean.let {
                        recyclerView.scroll(this@OneBookDetailsActivity, mData.indexOf(it) + 1, false)
                    }
            }
        })
        ...
        iv_setting.setOnClickListener {
            if (intent.getBooleanExtra("token", false)) {
                startActivityForResult(Intent(this@OneBookDetailsActivity, AddBookItemActivity::class.java).putExtra("data", bookData), ADMIN_EDIT_ITEM)
            } else
        ...
        headView.tvAddWish.setOnClickListener {
            transLayout.showProgress()
            if (bookData?.subscription?.subscription_type == 1) {//delete
                removeWish(bookData?.subscription?.id.toString()) {
                    transLayout.showContent()
                    it?.let {
                        if (it.code == 0) {
                            bookData?.subscription?.subscription_type = 0
                            headView.tvRead.isSelected = false
                            headView.tvAddWish.isSelected = false
                            headView.tvAddWish.text = resources.getString(R.string.string_book_wish)
                        } else
                            showToast(it.msg)
                    }
                }
                ...
                addWish(2, bookId, 1) {
                    ...
                    it?.let {
                        if (it.code == 0) {
                            //判断是否已经加入了想看, 删除想看
                            bookData?.subscription?.let { subscribe ->
                                if (subscribe.subscription_type == 2) {
                                    //清除选中
                                    headView.tvRead.text = resources.getString(R.string.string_book_readed)
                                }
                            }
                            headView.tvAddWish.text = resources.getString(R.string.string_book_had_wish)
                            headView.tvAddWish.isSelected = true
                            headView.tvRead.isSelected = false
                            if (null == bookData?.subscription) {
                                bookData?.subscription = SubscribeBean().apply {
                                    id = it.data.id
                                    subscription_type = 1
                                }
                            } else {
                                bookData?.subscription?.subscription_type = 1
                            }
                        } else
                            showToast(it.msg)
                    }
        ...
        headView.tvRead.setOnClickListener {
            transLayout.showProgress()
            if (bookData?.subscription?.subscription_type == 2) {
                removeWish(bookData?.subscription?.id.toString()) {
                    transLayout.showContent()
                    it?.let {
                        if (it.code == 0) {
                ...
                addWish(2, bookId, 2) {
                    ...
                    it?.let {
                        if (it.code == 0) {
                            ...
                            headView.tvRead.text = resources.getString(R.string.string_book_had_read)
                            ...
                            if (null == bookData?.subscription) {
                                ...
                                bookData?.subscription?.subscription_type = 2
                            }
                        } else
                            showToast(it.msg)
                    }
    ...
    private fun loginAdmin(pwd: String) {
        OkClientHelper.post(this, "admin/users/login", FormBody.Builder().add("confirmPasswd", pwd).build(), AdminLoginData::class.java, object : OkResponse {
            override fun success(result: Any?) {
                ...
                if (result.code == 0) {
                    SPUtils.setString(this@OneBookDetailsActivity, IConstant.ADMINTOKEN, result.data.token)
                    startActivityForResult(Intent(this@OneBookDetailsActivity, AddBookItemActivity::class.java).putExtra("data", bookData), ADMIN_EDIT_ITEM)
                    dialogPwd?.dismiss()
                } else {
    ...
    override fun request(flag: Int) {
        when (flag) {
            0 -> {
                OkClientHelper.get(this, "resources/2/$bookId", OneDetailsData::class.java, object : OkResponse {
                    override fun success(result: Any?) {
                        result as OneDetailsData
                        if (result.code == 0) {
                            if (result.data != null) {
                                bookData = result.data
                                headView.expendView.setTvShowText(result.data.book_intro)
                                Glide.with(this@OneBookDetailsActivity)
                                ...
                                headView.tvScore.text = "${result.data.book_score / 10f}"
                                headView.tvMovieType.text = result.data.book_author + "/" + result.data.book_publisher
                                headView.tvMovvieName.text = result.data.book_name
                                ...
                                result.data.subscription?.let {
                                    ...
                                    headView.tvRead.text = if (it.subscription_type == 2) resources.getString(R.string.string_book_had_read) else resources.getString(R.string.string_book_readed)
                                    headView.tvAddWish.text = if (it.subscription_type == 1) resources.getString(R.string.string_book_had_wish) else resources.getString(R.string.string_book_wish)
                                }
                ...
                transLayout.showProgress()
                OkClientHelper.get(this, "resources/2/$bookId/comment?sortType=$sortType&pageNo=$current&lastId=$lastId&recognition=1", VoiceData::class.java, object : OkResponse {
                    override fun success(result: Any?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_RECORD) {
                data?.let {
                    ...
                    LocalLogUtils.writeLog("OneBookDetails : 资源上传成功,请求socket 发送", System.currentTimeMillis())
