...
import kotlinx.android.synthetic.main.activity_recouce_appbar.*
import kotlinx.android.synthetic.main.movie_heard.*
...
class MovieActivity : BaseAct(), ItemOperator {

    //    private lateinit var headView: View
    private lateinit var adapter: QuickAdapter<BaseBean>
    private lateinit var heardAdapter: QuickAdapter<BaseSearchBean>
    ...
    private val topMoviesData by lazy {
        ArrayList<BaseSearchBean>()
    }

    override fun getLayoutId(): Int {
        return R.layout.activity_recouce_appbar
    }

    override fun initView() {
        recyclerReview.layoutManager = LinearLayoutManager(this)
//        headView = LayoutInflater.from(this).inflate(R.layout.movie_heard, recyclerReview, false)
        ...
        if (IConstant.USER_EXTROVERT.equals(character, true)) {// e 型
            iv_movie_banner.setImageResource(if (IConstant.HK.equals(sta, true) || IConstant.TW.equals(sta, true)) R.mipmap.icon_movie_baner_e_2 else R.mipmap.icon_movie_baner_e_1)
        } else { //I 型
            iv_movie_banner.setImageResource(if (IConstant.HK.equals(sta, true) || IConstant.TW.equals(sta, true)) R.mipmap.icon_movie_baner_i_2 else R.mipmap.icon_movie_baner_i_1)
        }
    }
    ...
    override fun initData() {
        ...
        adapter = object : QuickAdapter<BaseBean>(this, R.layout.item_voice, mData) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: BaseBean?) {
                item!!.resourceContent(helper!!, this@MovieActivity, glideUtil, defaultType)
                val voiceProgress = helper.getView(R.id.voiceProgress) as VoiceProgress
                voiceProgress.data = item
                item.echoClick(this@MovieActivity, helper) {
                ...
                helper.getView(R.id.relativeShare)?.setOnClickListener {
                    if (!TextUtils.isEmpty(item.is_shared))
                        shareDialog(item) { id ->
                            when (id) {
                                R.id.tv_Delete -> {
                                    DialogDeleteConment(this@MovieActivity).setHintText(resources.getString(R.string.string_delete_voice)).setOnClickListener(View.OnClickListener {
                                ...
                                R.id.tv_Self -> {
                                    if (item.is_private == 1) {
                                        ...
                                        DialogPrivacy(this@MovieActivity).setOnClickListener(View.OnClickListener {
                                            transLayout.showProgress()
                                            setVoicePrivacy(item)
                                        }).show()
                                    }
                        ...
                        DialogReport(this@MovieActivity).setResource(item.resource_type, item.subscription_id != 0).setOnClickListener(View.OnClickListener { report ->
                            when (report.id) {
                                R.id.tv_admin_setPrivacy -> {
                                    dialogPwd = DialogCommitPwd(this@MovieActivity).setOperator("isHidden", "1").setOnResultListener(DialogAdminPwdListener { key, value, pwd ->
                                        adminSetPrivacy(FormBody.Builder().add(key, value).add("confirmPasswd", pwd).build(), item)
                                    })
                                    dialogPwd?.show()
                                }
                                R.id.tv_Report ->
                                    DialogReportContent(this@MovieActivity).setOnResultListener(OnReportItemListener {
                                        transLayout.showProgress()
                                ...
                                R.id.tv_Follow -> {
                                    if (item.subscription_id == 0) {
                                        ...
                                        DialogGraffiti(this@MovieActivity).setTitle(String.format(resources.getString(R.string.string_follow_explor1), resources.getString(R.string.string_follow_movies)), resources.getString(R.string.string_confirm)).setOnClickListener(View.OnClickListener {
...
//                            transLayout.showProgress()
                ...
                helper.getImageView(R.id.roundImg)?.setOnClickListener { UserDetailsActivity.start(this@MovieActivity, item.user.avatar_url, item.user.id, helper.getImageView(R.id.roundImg)) }
                        ...
                        if (TextUtils.isEmpty(b.is_shared)) {
                            helper.getView(R.id.iv_Thumb).let {
                                ...
                                if (recyclerReview.isOffsetScreen(mData.indexOf(bean))) {
                                    SmallBang.attach2Window(this@MovieActivity).bang(it, 60f, null)
                                }
                            }
                        }
                    }
        ...
        heardAdapter = object : QuickAdapter<BaseSearchBean>(this, R.layout.item_movies_topic, topMoviesData) {
            ...
            override fun convert(helper: BaseAdapterHelper?, item: BaseSearchBean?) {
                ...
                when {
                    0 == helper!!.itemView.tag as Int -> params?.setMargins(AppTools.dp2px(this@MovieActivity, 15), 0, 0, 0)
                    topMoviesData.size - 1 == helper.itemView.tag as Int -> params?.setMargins(AppTools.dp2px(this@MovieActivity, 10), 0, AppTools.dp2px(this@MovieActivity, 15), 0)
                    else -> params?.setMargins(AppTools.dp2px(this@MovieActivity, 10), 0, 0, 0)
                }
                helper.itemView.layoutParams = params
                if (!TextUtils.isEmpty(item!!.movie_poster)) {
                    ...
                    Glide.with(this@MovieActivity)
                            .applyDefaultRequestOptions(RequestOptions()
                                    ...
                                    .signature(ObjectKey(item.movie_poster)))
                            .load(item.movie_poster)
                            .into(helper.getImageView(R.id.iv_Movies))
                    helper.getTextView(R.id.tvMoviesName).text = item.movie_title
                }
            }
        }
        recyclerMovieList.adapter = heardAdapter
    ...
    private fun down(item: BaseBean, isScroll: Boolean = true) {
        if (isScroll && recyclerReview.scrollState == RecyclerView.SCROLL_STATE_IDLE) {
            recyclerReview.scroll(this@MovieActivity, mData.indexOf(item) + 1)
        }
        ...
        try {
            ...
            if (file.exists()) {
                ...
                audioPlayer.start(if (SPUtils.getBoolean(this@MovieActivity, IConstant.ISEARPIECE, false)) AudioManager.STREAM_VOICE_CALL else AudioManager.STREAM_MUSIC)
            } else {
                OkClientHelper.downFile(this@MovieActivity, item.voice_url, { o ->
                    try {
                        if (null == o) {
                            showToast(resources.getString(R.string.string_error_file))
                            return@downFile
...
//                    showToast(VolleyErrorHelper.getMessage(it))
    ...
    private fun nextAudio() {
        playBean?.let {
            if (SPUtils.getBoolean(this@MovieActivity, IConstant.PLAY_MENU_AUTO, false)) {
                var index = mData.indexOf(it) + 1
                if (index >= mData.size) index = 0
                if (index == 0) recyclerReview.scrollToPosition(0)
    ...
    override fun initEvent() {
        ...
        customPlayMenu.setOnCircleMenuListener(object : OnCircleMenuOperatorListener {
            ...
            override fun top() {
                if (!audioPlayer.isPlaying) {
                    ...
                    playBean.let {
                        recyclerReview.scroll(this@MovieActivity, mData.indexOf(it) + 1, false)
                    }
            }
        })
        loadView.findViewById<View>(R.id.tvEmpty).setOnClickListener {
            DialogAlbumHowOperator(this).setTitle(resources.getString(R.string.string_what_follow_movies)).show()
        }
        heardAdapter.setOnItemClickListener { view, position ->
            OneMovieDetailsActivity.start(this, topMoviesData[position].movie_poster, topMoviesData[position].id, view.findViewById(R.id.iv_Movies))
        }
        linearList.setOnClickListener { startActivity(Intent(this, MovieListActivity::class.java)) }
        tv_Title.setOnClickListener { startActivity(Intent(this, SearchMovieActivity::class.java)) }
        ...
        tvMore.setOnClickListener { startActivity(Intent(this, AllHotMovieActivity::class.java)) }
        ...
        linear_userMovies.setOnClickListener {
            val loginBean = PreferenceTools.getObj(this, IConstant.LOCALTOKEN, LoginData.LoginBean::class.java)
        ...
        linear_similar.setOnClickListener {
            startActivity(Intent(this, SimilarMoviesActivity::class.java))
        }
    ...
    override fun request(flag: Int) {
        when (flag) {
            0 -> {//查询热门电影
                OkClientHelper.get(this, "resources/1?sortType=2&recentDay=30", SearchResData::class.java, object : OkResponse {
                    override fun success(result: Any?) {
                        topMoviesData.clear()
                        result as SearchResData
                        topMoviesData.addAll(result.data)
                        heardAdapter.notifyDataSetChanged()
                    }

                    override fun onFailure(any: Any?) {
                        if (!AppTools.isNetOk(this@MovieActivity)) {
                ...
                transLayout.showProgress()
                OkClientHelper.get(this, "resources/1/0/comment?sortType=$sortType&lastId=$lastId&pageNo=$flag&recognition=1", VoiceData::class.java, object : OkResponse {
                    override fun success(result: Any?) {
                        result as VoiceData
    ...
    private fun subscriber() {
        transLayout.showProgress()
        OkClientHelper.get(this, "voices/subscription?resourceType=1&lastId=$lastId", VoiceData::class.java, object : OkResponse {
            override fun success(result: Any?) {
                swipeRefresh.isRefreshing = false
                adapter.setLoadStatue(BaseQuickAdapter.ELoadState.EMPTY)
                result as VoiceData
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_RECORD) {
                data?.let {
                    ...
                    LocalLogUtils.writeLog("movieAct : 资源上传成功,请求socket 发送", System.currentTimeMillis())
