...
import org.xiaoxingqi.shengxi.wedgit.doodle.action.Action;
...
public class DoodleView extends SurfaceView implements SurfaceHolder.Callback, TransactionObserver {
    ...
    public void onResume() {
        new Handler(getContext().getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
//                Log.d("Mozator", "  DoodleView onResume");
...
//        Log.i(TAG, "surfaceView created, width = " + width + ", height = " + height);
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        isPause = true;
//        Log.d("Mozator", "surfaceDestroyed");
    }
    ...
    public void setPaintType(int type) {
        ...
        paintChannel.paintColor = Color.parseColor("#ff5f88");
        playbackChannel.paintColor = Color.parseColor("#08d9d6");
    }
    ...
    private void onMultiTransactionsDraw(List<Transaction> transactions) {
        ...
        if (isPause) {//避免出现 Activity 重复 onPause  onResume  造成死锁,
            for (Transaction t : transactions) {
                switch (t.getStep()) {
    ...
    public void isReceived(boolean isReceived) {
