...
public class SwipeBackHelper extends Handler {
    ...
    class ViewManager {
        ...
        private boolean addViewFromPreviousActivity() {
             ...
             * 判断当前界面是否加载webView
             */
            View temp = previousActivityContainer.getChildAt(0);
            if (mPreviousActivity instanceof WebViewActivity || mPreviousActivity instanceof ActionActivity || mPreviousActivity instanceof MainActivity) {
                Bitmap bitmap = Bitmap.createBitmap(temp.getWidth(), temp.getHeight(), Bitmap.Config.ARGB_4444);
                Canvas canvas = new Canvas(bitmap);
                temp.draw(canvas);
                mPreviousContentView = new View(mActivity);
                mPreviousContentView.setBackground(new BitmapDrawable(bitmap));
            } else {
                mPreviousContentView = temp;
